DROP TABLE IF EXISTS admissions;
DROP TABLE IF EXISTS d_hcpcs;
DROP TABLE IF EXISTS d_icd_diagnoses;
DROP TABLE IF EXISTS d_icd_procedures;
DROP TABLE IF EXISTS d_labitems;
DROP TABLE IF EXISTS diagnoses_icd;
DROP TABLE IF EXISTS drgcodes;
DROP TABLE IF EXISTS emar_detail;
DROP TABLE IF EXISTS emar;
DROP TABLE IF EXISTS hcpcsevents;
DROP TABLE IF EXISTS labevents;
DROP TABLE IF EXISTS microbiologyevents;
DROP TABLE IF EXISTS omr;
DROP TABLE IF EXISTS patients;
DROP TABLE IF EXISTS pharmacy;
DROP TABLE IF EXISTS poe_detail;
DROP TABLE IF EXISTS poe;
DROP TABLE IF EXISTS prescriptions;
DROP TABLE IF EXISTS procedures_icd;
DROP TABLE IF EXISTS provider;
DROP TABLE IF EXISTS services;
DROP TABLE IF EXISTS transfers;
DROP TABLE IF EXISTS caregiver;
DROP TABLE IF EXISTS chartevents;
DROP TABLE IF EXISTS d_items;
DROP TABLE IF EXISTS datetimeevents;
DROP TABLE IF EXISTS icustays;
DROP TABLE IF EXISTS ingredientevents;
DROP TABLE IF EXISTS inputevents;
DROP TABLE IF EXISTS outputevents;
DROP TABLE IF EXISTS procedureevents;
DROP TABLE IF EXISTS diagnosis;
DROP TABLE IF EXISTS edstays;
DROP TABLE IF EXISTS medrecon;
DROP TABLE IF EXISTS pyxis;
DROP TABLE IF EXISTS triage;
DROP TABLE IF EXISTS vitalsign;
DROP TABLE IF EXISTS radiology;
DROP TABLE IF EXISTS discharge;
DROP TABLE IF EXISTS patient_height;
DROP TABLE IF EXISTS patient_weight;
DROP TABLE IF EXISTS patient_bmi;
DROP TABLE IF EXISTS patient_eGFR;
DROP TABLE IF EXISTS patient_blood_pressure_sitting;
DROP TABLE IF EXISTS patient_blood_pressure_standing_1min;
DROP TABLE IF EXISTS patient_blood_pressure_standing_3mins;
DROP TABLE IF EXISTS patient_blood_pressure_standing;
DROP TABLE IF EXISTS patient_blood_pressure_lying;
DROP TABLE IF EXISTS patient_blood_pressure;
DROP TABLE IF EXISTS chartevents_d_items;
DROP TABLE IF EXISTS datetimeevents_d_items;
DROP TABLE IF EXISTS ingredientevents_d_items;
DROP TABLE IF EXISTS inputevents_d_items;
DROP TABLE IF EXISTS procedureevents_d_items;
DROP TABLE IF EXISTS outputevents_d_items;