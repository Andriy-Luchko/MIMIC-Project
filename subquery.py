...
class Subquery:
    ...
    def recursive_join(self, table_name, from_tables, where_conditions, visited_tables):
        ...
        for parent_table, join_condition in zip(current_table.parent_tables, current_table.join_conditions):
            where_conditions.append(f"{parent_table}.{join_condition[0]} = {table_name}.{join_condition[1]}")
    ...
    patient_subject_id_joins = ["admissions", "edstays", "patient_blood_pressure", "patient_blood_pressure_lying", 
                                ...
                                "patient_bmi", "patient_eGFR", "patient_height", "patient_weight", 
                                "labevents", "microbiologyevents", "radiology", "transfers"]
    
    # A list of table names to join starting from the "admissions" table, each containing data related to a hospital admission.
    admissions_hadm_id_joins = ["diagnoses_icd", "discharge", "drgcodes", "emar", "hcpcsevents", "icustays", 
    ...
    icustays_icu_stay_id_joins = ["outputevents", "datetimeevents", "chartevents", "ingredientevents", 
    ...
    def create_name_to_table_map(cls):
        ...
        name_to_table_map["d_items"] = Table(
            table_name="d_items", 
            parent_tables=["chartevents", "datetimeevents", "ingredientevents", "inputevents", "procedureevents", "outputevents"], 
            join_conditions=[("itemid", "itemid"), ("itemid", "itemid"), ("itemid", "itemid"), ("itemid", "itemid"), ("itemid", "itemid"), ("itemid", "itemid")]
        )
